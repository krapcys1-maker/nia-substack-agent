...
|---|---|---|
| maksimum 10 plików `.py` | **37 plików**, 38 208 wierszy | **PRZEKROCZONE** |
...
się testować bez przeglądarki i bez pieniędzy**. 207 zestawów
testów, 4645 sprawdzeń, żaden nie otwiera Chrome i żaden nie
...
284 wierszy, 9 funkcji na poziomie modułu, 0 klas
...
| `_o_tym_juz_pisalismy(wpis, rdzenie)` *(wewn.)* | Czy o tej pozycji z kanalu juz byla notka — po RDZENIACH, nie po adresie. |
...
| `_zapamietaj_wydarzenia(nowe, znane, ile, fakty)` *(wewn.)* | Zapisuje, ze o tych zdarzeniach material JUZ WROCIL. |
...
| `znajdz_ciekawostki(conn, run_id, ile, na_artykul)` | Materiał na notki w dni bez artykułu. |
...
| `scout(conn, run_id, count)` | Etap 1 — skaut tematow. |
...
6052 wierszy, 106 funkcji na poziomie modułu, 3 klas
