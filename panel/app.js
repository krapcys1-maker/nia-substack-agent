...
const field=(key,en,pl,kind='text',options={})=>{
  ...
  const attr=`data-field="${key}" data-kind="${options.format||kind}"`;
...
function settingsView() {
  ...
  <section><h2>${t('Publishing and interactions','Publikacje i interakcje')}</h2>${field('wolumeny.notki_dziennie','Notes per day','Notki dziennie','number')}${field('wolumeny.artykuly_tygodniowo','Articles per week','Artykuły tygodniowo','number')}
